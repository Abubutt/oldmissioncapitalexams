...
// Baseline share of the exam each section should get in adaptive mode,
// independent of the user's own accuracy — from a firsthand account of the
// actual screening (2026-07): "80% discrete math [combinatorics/permutations,
// not proofs], then some C++ code snippets, OS stuff, and Linux commands."
// Index-aligned with SECTION_NAMES. This only steers the *default* adaptive
// mix; picking "Discrete Math only" in the Section dropdown ignores this
// entirely and goes 100% discrete math, which is the more direct way to
// prep for what's actually reported as the bulk of the real exam.
const SECTION_EXAM_WEIGHT = [
  1, // C++
  0.4, // Python
  0.4, // Data Structures & Algorithms
  1, // Operating Systems
  1, // Linux / CLI
  0.3, // Networking & General CS
  6 // Discrete Math
];

/**
 * Split totalQuestions across the active sections (focusSections, or every
 * section if null/empty). Every active section gets at least MIN_PER_SECTION
 * (scaled down if totalQuestions is too small to support that floor across
 * all of them); the remainder is allocated proportionally to
 * SECTION_EXAM_WEIGHT × weakness (1 - accuracy) — sections that are both
 * more heavily represented on the real exam and ones the user is worse at
 * get more new questions. Sections with no history default to a neutral 0.5
 * accuracy.
 *
 * With a single-section (or any proper subset) focusSections, the mix is
 * still weighted the same way but confined to just the chosen sections —
 * "discrete math + C++ only, weighted toward discrete math" rather than
 * either the full broad mix or a single section.
 */
function allocateQuestionCounts(history, focusSections, totalQuestions = DEFAULT_TOTAL_QUESTIONS) {
  const allIndices = SECTION_NAMES.map((_, i) => i);
  const active =
    Array.isArray(focusSections) && focusSections.length > 0
      ? focusSections.filter((i) => Number.isInteger(i) && i >= 0 && i < SECTION_NAMES.length)
      : allIndices;
  const activeSections = active.length > 0 ? active : allIndices;
  ...
  const accuracies = activeSections.map((i) => {
    const sec = history.sections[SECTION_NAMES[i]];
  ...
  const weights = accuracies.map((acc, idx) => (SECTION_EXAM_WEIGHT[activeSections[idx]] ?? 1) * Math.max(0.05, 1 - acc));
  ...
  for (let k = 0; k < leftover; k++) {
    extra[byFraction[k % n].idx]++;
  }

  return activeSections.map((sIndex, idx) => ({
    name: SECTION_NAMES[sIndex],
    sIndex,
    count: perSectionFloor + extra[idx]
  }));
}

const DIFFICULTY_GUIDANCE = {
  easy: 'Introductory and fundamental: concepts a junior engineer would know from a first course or a few weeks of hands-on practice. Favor direct recall or a single obvious reasoning step over multi-step reasoning. Distractors should be plausible but clearly distinguishable on reflection — avoid subtle near-misses, edge-case gotchas, or trick questions.',
  medium: 'Standard screening difficulty: the broad CS-fundamentals level expected of a junior software engineer. Requires real understanding, not just recall, but avoids obscure edge cases or trick questions.',
  hard: "Genuinely hard: subtle edge cases, common misconceptions, or behavior a junior engineer would only know from real experience, not just coursework. Distractors should be genuinely tempting — plausible misconceptions or almost-correct answers, not obviously wrong options. Favor questions that require tracing through a code snippet, reasoning about edge-case or undefined behavior, or distinguishing between subtly different alternatives, where the section allows it."
};

// Per-section style notes beyond the generic instructions below — currently
// just Discrete Math, added after a firsthand account of the real exam
// specified it's computational counting problems, not proof-writing.
const SECTION_STYLE_NOTES = {
  'Discrete Math':
    'This section is computational combinatorics and permutations: the multiplication/counting principle, permutations vs. combinations, arrangements with repetition, circular permutations, the pigeonhole principle, and similar — each solvable by applying a formula or counting argument to reach a single numeric or formula-recognition answer. Do NOT ask for formal proofs, proof sketches, or "show that" style questions. Avoid graph theory, formal logic, and set-theoretic proofs — this is counting, not graduate discrete math.'
};
...
function buildSystemPrompt(sectionName, count, materialExcerpt, materialMode, difficulty) {
  const guidance = DIFFICULTY_GUIDANCE[difficulty] || DIFFICULTY_GUIDANCE.medium;
  const styleNote = SECTION_STYLE_NOTES[sectionName];
...
// against a verbose outlier response. Sized generously enough to cover
// reasoning models (o4-mini and others bill hidden "reasoning tokens" before
// the visible JSON output — measured ~2000-2900 total completion tokens for
// an 8-to-20-question batch, mostly fixed per-call reasoning overhead rather
// than scaling linearly with count) as well as plain chat models (~50-65
// tokens/question on gpt-4o). A model that hits this ceiling mid-reasoning
// returns no output at all rather than a truncated one (observed with gpt-5
// at the old 8192 cap during testing), so err generous — it's free to leave
// unused.
function estimateMaxTokens(count) {
  return Math.min(40000, count * 200 + 4000);
}
...
async function generateSection(provider, name, sIndex, count, recentQuestions, materialExcerpt, materialMode, difficulty) {
  const system = buildSystemPrompt(name, count, materialExcerpt, materialMode, difficulty);
...
async function generateExam(
  userKey,
  { materialMode = 'blend', focusSections = null, totalQuestions = DEFAULT_TOTAL_QUESTIONS, difficulty = 'medium' } = {}
) {
  ...
  const allocations = allocateQuestionCounts(history, focusSections, clampedTotal);
